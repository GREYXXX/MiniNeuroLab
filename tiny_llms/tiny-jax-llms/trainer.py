...
def eval_step(state, input_ids, target_ids):
    ...
    loss, metrics = compute_loss(
        state.params, state.apply_fn, input_ids, target_ids, None, deterministic=True
    )
    return loss, metrics
...
def generate_text(
    ...
    context_size = state.params["params"]["Embed_1"]["embedding"].shape[
        0
    ]  # pos_embed size
...
class SimpleTrainer:
    ...
    def train(self, text_data: str, tokenizer):
        ...
        print(
            f"Train samples: {len(data['train_inputs'])}, Val samples: {len(data['val_inputs'])}"
        )
        ...
        print(
            f"Training for {self.train_config.num_epochs} epochs, {steps_per_epoch} steps each"
        )
        ...
        for epoch in range(self.train_config.num_epochs):
            ...
            for batch_idx in range(steps_per_epoch):
                ...
                state, loss, metrics = train_step(
                    state, batch_inputs, batch_targets, step_rng
                )
                ...
                if step % self.train_config.eval_every == 0:
                    val_metrics = self.evaluate(
                        state, data["val_inputs"], data["val_targets"]
                    )
                    pbar.write(
                        f"Step {step} - Val Loss: {val_metrics['loss']:.3f}, Val Acc: {val_metrics['accuracy']:.3f}"
                    )
...
def main():
    ...
    train_config = TrainingConfig(batch_size=8, num_epochs=3, eval_every=50)
    ...
    for prompt in prompts:
        generated = generate_text(
            state, tokenizer, prompt, max_new_tokens=20, temperature=0.8
        )
