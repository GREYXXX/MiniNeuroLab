...
class GELU(nn.Module):
    ...
    def __call__(self, x):
        return (
            0.5
            * x
            * (1 + jnp.tanh(jnp.sqrt(2.0 / jnp.pi) * (x + 0.044715 * jnp.power(x, 3))))
        )
...
class MultiHeadAttention(nn.Module):
    ...
    def __call__(self, x, deterministic=True):
        ...
        q = nn.Dense(self.config.emb_dim, use_bias=self.config.qkv_bias, name="q_proj")(
            x
        )
        k = nn.Dense(self.config.emb_dim, use_bias=self.config.qkv_bias, name="k_proj")(
            x
        )
        v = nn.Dense(self.config.emb_dim, use_bias=self.config.qkv_bias, name="v_proj")(
            x
        )
        ...
        if not deterministic:
            attn_weights = nn.Dropout(self.config.drop_rate)(
...
class TransformerBlock(nn.Module):
    ...
    def __call__(self, x, deterministic=True):
        ...
        attn_out = MultiHeadAttention(self.config)(
            LayerNorm()(x), deterministic=deterministic
        )
        if not deterministic:
            attn_out = nn.Dropout(self.config.drop_rate)(
        ...
        if not deterministic:
            ff_out = nn.Dropout(self.config.drop_rate)(
                ff_out, deterministic=deterministic
            )
        x = x + ff_out
