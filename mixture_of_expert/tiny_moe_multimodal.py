...
class ToyImageTextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = torch.randn(3, self.cfg.img_size, self.cfg.img_size) * 0.2 + concept["img_mean"]
...
class MoELayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        # x: (B, d_model)
        B, D = x.shape
        topk_idx, norm_weights, gate_probs, hard_mask = self.gate(x)  # idx: (B, k), weights: (B, k)
...
def clip_style_loss(logits_per_img: torch.Tensor, logits_per_txt: torch.Tensor) -> torch.Tensor:
...
def train(cfg: Config):
    ...
    dl = DataLoader(ds, batch_size=cfg.batch_size, shuffle=True, num_workers=0, collate_fn=collate, drop_last=True)
        ...
        for step, (imgs, tokens, mask) in enumerate(dl, 1):
            ...
            if step % 50 == 0:
                with torch.no_grad():
                    ...
                    print(f"Epoch {epoch+1} Step {step}: loss={total/step:.4f} aux={total_aux/step:.4f}")

        print(f"Epoch {epoch+1} done. Avg loss={total/len(dl):.4f}, aux={total_aux/len(dl):.4f}")
    ...
    with torch.no_grad():
        ...
        neg = (logits_i.sum() - logits_i.diag().sum()) / (logits_i.numel() - imgs.size(0))
